...
class UserSubscription(SQLModel, table=True):
    ...
    variant_id: str = Field(index=True)     # Which plan are they on? (explorer(free), creator, professional)
    
    # Status Tracking
    status: str             # active, past_due, cancelled, expired, etc.
    current_period_end: datetime = Field(nullable=True) # When does access actully end?
